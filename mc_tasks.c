...
__weak void MCboot( MCI_Handle_t* pMCIList[NBR_OF_MOTORS] )
{
  ...
  else
  {
    ...
    /**************************************/
    RUC_Init(&RevUpControlM1,pSTC[M1],&VirtualSpeedSensorM1, &Bemf_M1, pwmcHandle[M1]);
          ...
          {
            {
             ...
             ThreePwm_TurnOnLowSides(pwmcHandle[M1]);
...
__weak void SixStep_Clear(uint8_t bMotor)
{
  ...
  SixStepVars[bMotor].DutyCycleRef = (int16_t)0;
...
inline uint16_t SixStep_StatorController(void)
{
  ...
  if ((true == Bemf_ADC_M1.ZcDetected) && (RUN == Mci[M1].State))
  {
    RCM_ExecNextConv();
  }
  ...
  ThreePwm_LoadNextStep( &PWM_Handle_M1 );
